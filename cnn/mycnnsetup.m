function net = mycnnsetup(net, x, y,data)  
    inputmaps = 1;  
    % B=squeeze(A) 返回和矩阵A相同元素但所有单一维都移除的矩阵B，单一维是满足size(A,dim)=1的维。  
    % train_x中图像的存放方式是三维的reshape(train_x',28,28,样本)，前面两维表示图像的行与列，  
    ...
    for l = 1 : numel(net.layers)   %  layer  
        if strcmp(net.layers{l}.type, 's') % 如果这层是 子采样层(池化层 pooling）  
            % subsampling层的mam psize，最开始mapsize是每张图的大小28*28  
        ...
        if strcmp(net.layers{l}.type, 'c') % 如果这层是 卷积层  convn
