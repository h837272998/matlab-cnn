function net = cnnbp(net, y)  
    ...
    % 残差 反向传播回前一层,net.fvd保存的是残差 
    net.fvd = (net.ffW' * net.od);              %  feature vector delta  
    if strcmp(net.layers{n}.type, 'c')         %  only conv layers has sigm function  
        net.fvd = net.fvd .* (net.fv .* (1 - net.fv)); %net.fv是前一层的输出，作为输出层的输入
    end  
    ...
    for j = 1 : numel(net.layers{n}.a) % 最后一层的特征map的个数  
        ...
        net.layers{n}.d{j} = reshape(net.fvd(((j - 1) * fvnum + 1) : j * fvnum, 1), sa(1), sa(2), 1);  
    end  
    ...
    for l = (n - 1) : -1 : 1  
        if strcmp(net.layers{l}.type, 'c')  
            for j = 1 : numel(net.layers{l}.a) % 该层特征map的个数  
                ...
                % end  
                net.layers{l}.d{j} = net.layers{l}.a{j} .* (1 - net.layers{l}.a{j}) .* (expand(net.layers{l + 1}.d{j}, [net.layers{l + 1}.scale net.layers{l + 1}.scale ]) / net.layers{l + 1}.scale ^ 2);  
            end  
